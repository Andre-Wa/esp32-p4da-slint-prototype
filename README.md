...
| Orientação (retrato nativo → paisagem) e ordem de bytes da cor | ✅ Resolvido. Painel é 480×800 nativo; a UI é 800×480 (paisagem) via `.rotation = Rotate90`. **Pegadinha da API do Slint:** com rotação ativa, `.size` passado pro `SlintPlatformConfiguration` deve ser o tamanho **lógico pós-rotação** (800×480), não a resolução física nativa do painel (480×800) — o Slint transpõe isso internamente pra escrever no buffer físico correto. `byte_swap = false` resolveu a inversão de cor (o barramento DPI do P4 espera RGB565 little-endian nativo, não big-endian como o Slint assume por padrão) | Nenhuma ação — já funcionando |
